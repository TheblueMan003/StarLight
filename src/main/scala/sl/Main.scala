...
object Main {
        ...
        args(0) match
          ...
          case "doc" => {
            if (args.length < 2 && lastBuild == null) then {
              ...
              val libraries: List[String] = FileUtils
                .getListOfFiles(path)
                .filterNot(_.contains("__init__.sl"))
              libraries.par.foreach(f =>
                makeDocumentation(
                  f.dropRight(3)
                    .replaceAllLiterally("\\", "/")
                    .replaceAllLiterally(path, ""),
                  List(f)
                )
              )
              val names = libraries.map(f =>
                f.dropRight(3)
                  .replaceAllLiterally("\\", "/")
                  .replaceAllLiterally(path, "")
              )
          ...
          case "install" => {
            if (args.length == 2) {
              ...
              Reporter.error(
                f"Expected 1 or 2 argument got: ${args.length - 1}"
              )
            }
          }
          case "update" => {
          ...
          case "clearcache" | "clearcaches" | "cacheclear" | "cachesclear" |
              "cc" => {
            FileUtils.deleteDirectory("./bin")
          ...
          case ">" | "show" => {
            if (args.length != 2) then {
              ...
              if (name.contains("*")) {
                lastIR
                  .filter(f =>
                    f.name.contains(name.replaceAllLiterally("*", ""))
                  )
                  .foreach { f => f.print() }
              } else {
                lastIR.filter(f => f.name == name).headOption match
                  ...
                  case None => {
                    lastIR.filter(f => f.name.contains(name)).foreach { f =>
                      f.print()
                    }
                  }
              }
          ...
          case "?" => {
            if (args.length != 2) then {
              ...
              if (name.contains("*")) {
                varis
                  .filter(f =>
                    f.fullName.contains(name.replaceAllLiterally("*", ""))
                  )
                ...
                varis.filter(f => f.fullName == name).headOption match
                  ...
                  case None => {
                    varis.filter(f => f.fullName.contains(name)).foreach { f =>
                      println(f.fullName)
                    }
                  }
              }
          ...
          case "help" => {
            println(
              "build <config_name>: Build the project with the config contains in the file config_name. The .slconf must be omitted."
            )
            println("clear: Clear the console")
            println("clearcache: Clear the caches of the compiler")
            println(
              "debug <function>: Interpret the function in debug mode (print every operation). The .sl must be omitted and the internal name must be used. Example: `main.ticking.main`. You must compile the project before."
            )
            println("exit: Close")
            ...
            println(
              "update <library>: Update a library into the local project."
            )
            println(
              "updateconfig <config_name>: Update the config file contains in the file config_name (Add the new field from new version). The .slconf must be omitted."
            )
            println(
              "watch: Watch the project and compile it when a file change"
            )
          }
          case "exit" => {
            FileUtils.deleteDirectory("./bin")
          ...
          case "tree" => {
            FileUtils.safeWriteFile(
              "tree.txt",
              List(lastContxt.asPrettyString(""))
            )
  ...
  def newProject(args: Array[String]): Unit = {
    ...
    FileUtils.safeWriteFile(
      directory + "/java.slconf",
      ConfigLoader.get("java", name, namespace, author)
    )
    FileUtils.safeWriteFile(
      directory + "/bedrock.slconf",
      ConfigLoader.get("bedrock", name, namespace, author)
    )
    FileUtils.safeWriteFile(
      directory + "/src/main.sl",
      List("package main", "", "def ticking main(){", "", "}")
    )

    FileUtils.createDirectory(directory + "/lib")
    FileUtils.createDirectory(directory + "/java_datapack/data")
    FileUtils.safeWriteFile(
      directory + "/java_datapack/pack.mcmeta",
      List(MCJava.getPackMeta())
    )
    FileUtils.createDirectory(directory + "/bedrock_datapack")

    FileUtils.copyFromResourcesToFolder(
      "icon/64.png",
      directory + "/java_resourcepack/pack.png"
    )
    FileUtils.copyFromResourcesToFolder(
      "icon/256.png",
      directory + "/bedrock_resourcepack/pack_icon.png"
    )

    FileUtils.copyFromResourcesToFolder(
      "icon/64.png",
      directory + "/java_datapack/pack.png"
    )
    FileUtils.copyFromResourcesToFolder(
      "icon/256.png",
      directory + "/bedrock_datapack/pack_icon.png"
    )

    FileUtils.copyFromResourcesToFolder(
      "configs/blockmap.csv",
      directory + "/configs/blockmap.csv"
    )
    FileUtils.copyFromResourcesToFolder(
      "configs/soundmap.csv",
      directory + "/configs/soundmap.csv"
    )
    FileUtils.copyFromResourcesToFolder(
      "configs/color.csv",
      directory + "/configs/color.csv"
    )
  ...
  def build(args: String): Unit = {
    ...
    Settings.version =
      List(Settings.version(0), Settings.version(1), Settings.version(2) + 1)

    val libs =
      FileUtils.getListOfTopDirectories("./lib").map(l => "./lib/" + l + "/src")
    val datpack =
      if (Settings.target == MCJava)
        FileUtils
          .getListOfTopDirectories("./lib")
          .map(l => "./lib/" + l + "/java_datapack")
      else
        FileUtils
          .getListOfTopDirectories("./lib")
          .map(l => "./lib/" + l + "/bedrock_datapack")
    val respack =
      if (Settings.target == MCJava)
        FileUtils
          .getListOfTopDirectories("./lib")
          .map(l => "./lib/" + l + "/java_resourcepack")
      else
        FileUtils
          .getListOfTopDirectories("./lib")
          .map(l => "./lib/" + l + "/bedrock_resourcepack")

    if (Settings.target == MCJava) {
      compile(
        args,
        "./src" :: libs,
        "./java_datapack" :: datpack,
        "./java_resourcepack" :: respack,
        Settings.java_datapack_output
      )
    }
    if (Settings.target == MCBedrock) {
      compile(
        args,
        "./src" :: libs,
        "./bedrock_datapack" :: datpack,
        "./bedrock_resourcepack" :: respack,
        Settings.bedrock_behaviorpack_output
      )
    }
  ...
  def compile(
    ...
    Reporter.info(
      f"Number of mcfunctions: ${output.filter(_.getPath().endsWith(".mcfunction")).size}"
    )
    Reporter.info(
      f"Number of commands: ${output.filter(_.getPath().endsWith(".mcfunction")).map(_.getContents().size).foldRight(0)(_ + _)}"
    )
      ...
      Settings.java_resourcepack_output
        ...
        .foreach(f =>
          ResourcePackBuilder.build(
            resourceInput,
            f,
            Settings.target.getResourcesExtraFiles(context) ::: context
              .getAllJsonFiles()
              .filter(_.isJavaRP())
              .map(f => (f.getIRFile()))
          )
    ...
    FileUtils.safeWriteFile(
      "vscode/token",
      Utils.getConfig("blocks.txt") ::: Utils
        .getConfig("sounds/java.csv")
        .map("minecraft:" + _) ::: Utils.getConfig("particles.txt") ::: context
        .getObjects()
    )
  ...
  def makeDocumentation(name: String, inputs: List[String]) = {
    ...
    FileUtils.safeWriteFile(
      f"./docs/libraries/${name}.md",
      List(DocMaker.make2(InstructionList(tokenized.map(_.get))))
    )
  }
}
