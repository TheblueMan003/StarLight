...
object ResourcePackBuilder{
    def build(source: String, target: String, jsonFiles: List[objects.JSONFile])={
    ...
    def makeRPFolder(source: String, target: String, jsonFiles: List[objects.JSONFile])={
        FileUtils.deleteDirectory(target)
        getListOfRPFiles(source, "").map(file =>{
    ...
    def makeRPZip(source: String, target: String, jsonFiles: List[objects.JSONFile])={
        val Buffer = 2 * 1024
        ...
        // Copy Files
        getListOfRPFiles(source, "").foreach { name =>
