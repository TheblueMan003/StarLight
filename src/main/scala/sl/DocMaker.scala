...
object DocMaker {
    ...
    def formatTree(
      ...
      val directoryTree = directories
        ...
        .map((key, values) => {
          (
            key,
            s"$indent- **${key.replace("/", "")}**\n" + formatTree(
              values.map(f => (f._2, f._3)),
              indent + "\t"
            )
          )
        })
      ...
      val fileTree = remainingFiles
        .map(file => s"$indent- [${file._1}](libraries/${file._2}.md)\n")
        .mkString
  ...
  /** Traverse the instruction tree and generate the html documentation from
  ...
  def make2(instr: Instruction)(implicit packag: String = ""): String = {
    instr match{
      ...
      case FunctionDecl(name, block, typ, args, List(), modifier)
          if modifier.protection != Private =>
        ContentMaker.h2(modifier.schema() + " function " + name) +
          ContentMaker.p(
            "### Arguments:\n" + args
              .map(a => f"- ${a.name} (`${a.typ}`)")
              .mkString("\n") + f"\n### Return:\n- $typ"
          ) + "\n\n\n" +
      ...
      case StructDecl(name, List(), block, modifier, parent)
          if modifier.protection != Private =>
      ...
      case StructDecl(name, generics, block, modifier, parent)
          if modifier.protection != Private =>
        ContentMaker.h1(
          modifier.schema() + " struct " + name + generics.mkString(
            "<",
            ",",
            ">"
          )
      ...
      case ClassDecl(
        ...
        ContentMaker.h1(
          modifier.schema() + " class " + name + generics.mkString(
            "<",
            ",",
            ">"
          )
      ...
      case TemplateDecl(name, block, modifier, parent, generics, parentGenerics)
          if modifier.protection != Private =>
        ContentMaker.h1(
          modifier.schema() + " template " + name + generics.mkString(
            "<",
            ",",
            ">"
          )
      ...
      case InstructionList(list) =>
        list.map(make2).reduceOption(_ + _).getOrElse("")
